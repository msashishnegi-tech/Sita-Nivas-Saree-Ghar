...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!products || products.length === 0) { products = DEFAULT_PRODUCTS; await kv.set('products', products); }
    ...
    if (req.method === 'GET') {
      ...
      products.push(np); await kv.set('products', products);
      res.status(201).json({ success: true, product: np });
      ...
      products[idx] = { ...products[idx], ...u }; await kv.set('products', products);
...
"Commit changes"
📋 Step 4: api/admin/products.js replace karo
Same process — api/admin/products.js kholo, edit karo, purana delete karo, yeh paste karo:

const { kv } = require('@vercel/kv');

const DEFAULT_PRODUCTS = [
  { id: "1", name: "Premium Silk Kurta", price: 2999, salePrice: 1999, category: "men", image: "https://images.unsplash.com/photo-1594938298603-c8148c4dae35?w=500", description: "Premium quality silk kurta", stock: 50, createdAt: "2026-07-22T00:00:00Z" },
  { id: "2", name: "Floral Maxi Dress", price: 3499, salePrice: 2499, category: "women", image: "https://images.unsplash.com/photo-1572804013309-59a88a7e9b60?w=500", description: "Beautiful floral print maxi dress", stock: 30, createdAt: "2026-07-22T00:00:00Z" },
  { id: "3", name: "Denim Jacket Classic", price: 4999, salePrice: 3499, category: "men", image: "https://images.unsplash.com/photo-1551028719-00167b16eac5?w=500", description: "Classic denim jacket", stock: 25, createdAt: "2026-07-22T00:00:00Z" },
  { id: "4", name: "Designer Lehenga Set", price: 8999, salePrice: 6999, category: "women", image: "https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=500", description: "Elegant designer lehenga", stock: 15, createdAt: "2026-07-22T00:00:00Z" }
];
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') { res.json(products); }
    else if (req.method === 'POST') {
      const np = { id: Date.now().toString(), ...req.body, createdAt: new Date().toISOString() };
      np.price = Number(np.price); if (np.salePrice) np.salePrice = Number(np.salePrice); np.stock = Number(np.stock) || 0;
      products.push(np); await kv.set('products', products);
